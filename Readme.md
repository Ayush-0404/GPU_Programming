...
![Build Output](Execution_Proof/Execution_terminal_output.png)
...
```
┌──────────┐    ┌──────────────┐    ┌─────────────┐    ┌──────────┐    ┌──────────┐
│ Load RGB │ →  │ cudaMemcpy   │ →  │ Grayscale   │ →  │ Box Blur │ →  │ Save PNG │
│ from disk│    │ Host→Device  │    │ Kernel (1D) │    │ Kernel(2D)│    │ to disk  │
└──────────┘    └──────────────┘    └─────────────┘    └──────────┘    └──────────┘
                                     1 thread/pixel     16x16 blocks
```
